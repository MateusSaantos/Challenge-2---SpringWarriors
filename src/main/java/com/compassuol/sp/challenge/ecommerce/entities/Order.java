...
public class Order implements Serializable {
    ...
    public Order(Address address, Payment paymentMethod, Float subtotalValue,
                 Float discount,List<ProductInOrder> order) {

        this.order = order;
        this.address = address;
        this.paymentMethod = paymentMethod;
        this.status = Status.CONFIRMED;
        //this.subtotalValue = 0F;
        this.discount = discount;
        this.createdDate = LocalDateTime.parse(Instant.now().toString());
        this.cancelDate = null;
        this.cancelReason = null;

        calculateSubtotalValue(order);
        calculateTotalValue();
